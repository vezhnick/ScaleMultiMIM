function [rounds_vs_max measurements cov_evolve it_is_const] = SearchGridGPfixedK(init_points, finest_grid, full_measurements, iter_num, beta, adapt)
if(isempty(beta))
    beta = 0.1;
end

grid = finest_grid(:,init_points);
measurements = full_measurements(:, init_points);

plotting = true;

noise_level = 8;

[x_new F_app sig x_ind hyp] = BayesOpt(grid', measurements', finest_grid','minimize','true','covFunc','covSEiso','length',-20, 'beta', beta);

cov_evolve = hyp.cov;
hyp.cov = hyp.cov * 0 + 1;
hyp.cov(end) = hyp.cov(end) * 1.2;

[x_new F_app sig x_ind hyp] = BayesOpt(grid', measurements', finest_grid','minimize','true','covFunc','covSEiso','length',-0, 'beta', beta, 'hyp', hyp);
...
%     plot(init_points, full_measurements(init_points),  'r*')
%     pause;
% end
it_is_const = 0;
for i = 1 : iter_num
...
%         hold off
%         subplot(3,1,3)
        plot(F_app, 'LineWidth',2);
        hold on
        plot(full_measurements, 'black', 'LineWidth',2);
        plot(F_app + sig * beta,'g', 'LineWidth',2);                    
        plot(find(grid_known), full_measurements(grid_known),  'r+', 'MarkerSize', 10)            
        plot(x_ind, full_measurements(x_ind),  'ro', 'MarkerSize', 10)
        plot(init_points, full_measurements(init_points),  'r+', 'MarkerSize', 10)
        hold off
%         subplot(3,1,1)
    end
