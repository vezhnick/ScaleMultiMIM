function [TestPrediction, Neibs] = TagProp(ImagesDB_train, ImagesDB_test, k)
...
% ImagesDB_test = ImagesDB(2:2:end);
% ImagesDB_train = ImagesDB(1:2:end);
% 
% delete ImagesDB;

TestPrediction = zeros(length(ImagesDB_test), 33);
TestLabels = zeros(length(ImagesDB_test), 33);
...
%     kNN = kNN(idx);
    ...
    for c = 1 : 33
        if(ismember(c, cur_test_im.labels))% && pred_labels(c) > 0.01)
            recall(c) = recall(c) + pred_labels(c);
        end
    end
end
